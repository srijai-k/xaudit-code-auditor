...
const CORPUS = [
    ...
    { id: "xss-location-href-javascript-uri", group: "xss", bucket: "vulnerable", expectRuleIds: ["xss-location-javascript-uri"], note: "new sink, previously a documented gap (fixed 2026-09-03): location.href built by concatenating the literal javascript: scheme with a dynamic value",
      code: `function go(userInput) {\n  location.href = "javascript:" + userInput;\n}\n` },
    { id: "xss-window-location-javascript-uri-one-hop", group: "xss", bucket: "vulnerable", expectRuleIds: ["xss-location-javascript-uri"], note: "new sink: window.location assigned a javascript: URI traced back one variable hop",
      code: `function go(userInput) {\n  const dest = "javascript:" + userInput;\n  window.location = dest;\n}\n` },
    { id: "xss-setattribute-href-javascript-uri", group: "xss", bucket: "vulnerable", expectRuleIds: ["xss-setattribute-javascript-uri"], note: "new sink, previously a documented gap (fixed 2026-09-03): setAttribute('href', ...) with a javascript: URI built from a dynamic value",
      code: `function go(el, userInput) {\n  el.setAttribute('href', 'javascript:' + userInput);\n}\n` },
    { id: "xss-jsx-href-javascript-uri-template", group: "xss", bucket: "vulnerable", expectRuleIds: ["xss-jsx-href-javascript-uri"], note: "new sink: a JSX href prop built as a template literal starting with the javascript: scheme",
      code: `export default function Link({ payload }) {\n  return <a href={\`javascript:\${payload}\`}>Click</a>;\n}\n` },
    { id: "xss-location-href-plain-redirect-not-flagged", group: "xss", bucket: "safe", forbidSeverity: "high", note: "the entire reason this check is scoped narrowly: an ordinary, extremely common redirect with no javascript: literal anywhere must NOT be flagged, even though the value is fully dynamic",
      code: `function goToProfile(userId) {\n  location.href = "/profile/" + userId;\n}\n` },
    { id: "xss-location-href-external-url-not-flagged", group: "xss", bucket: "safe", forbidSeverity: "high", note: "a normal external-URL redirect built dynamically must NOT be flagged — no javascript: scheme literal present",
      code: `function redirectTo(path) {\n  window.location.href = "https://example.com" + path;\n}\n` },
    { id: "xss-setattribute-href-normal-url-not-flagged", group: "xss", bucket: "safe", forbidSeverity: "high", note: "setAttribute('href', ...) with an ordinary dynamic URL (no javascript: literal) must NOT be flagged",
      code: `function setLink(el, slug) {\n  el.setAttribute('href', '/posts/' + slug);\n}\n` },
    { id: "xss-jsx-href-normal-not-flagged", group: "xss", bucket: "safe", forbidSeverity: "high", note: "a normal JSX href prop built dynamically, no javascript: scheme, must NOT be flagged",
      code: `export default function UserLink({ username }) {\n  return <a href={\`/users/\${username}\`}>{username}</a>;\n}\n` },
    ...
    // ===================== Weak authentication (new rule group) =====================
    { id: "auth-hardcoded-password-comparison", group: "auth", bucket: "vulnerable", expectRuleIds: ["auth-hardcoded-credential-comparison"], note: "a request password compared directly against a hardcoded literal — a real backdoor/debug-credential shape",
      code: `function login(req) {\n  if (req.body.password === "admin123") {\n    return grantAccess();\n  }\n  return denyAccess();\n}\n` },
    { id: "auth-hardcoded-apikey-comparison", group: "auth", bucket: "vulnerable", expectRuleIds: ["auth-hardcoded-credential-comparison"], note: "an API key checked against a literal instead of a configured value (also matches the OpenAI vendor secret pattern -- both rules are expected to fire independently on the same literal)",
      code: `function checkApiKey(apiKey) {\n  return apiKey === "sk-hardcoded-secret-key-12345";\n}\n` },
    { id: "auth-jwt-decode-no-verify", group: "auth", bucket: "vulnerable", expectRuleIds: ["auth-jwt-decode-without-verify"], note: "jwt.decode() used with no jwt.verify() anywhere in the file — the signature is never checked",
      code: `import jwt from 'jsonwebtoken';\nfunction getUserId(token) {\n  const decoded = jwt.decode(token);\n  return decoded.userId;\n}\n` },
    { id: "auth-password-compared-to-hash-variable", group: "auth", bucket: "safe", forbidSeverity: "medium", note: "comparing to a variable (presumably a hash), not a literal — the normal, correct shape — must NOT flag",
      code: `function checkLogin(user, hashedInput) {\n  return user.password === hashedInput;\n}\n` },
    { id: "auth-role-compared-to-literal-not-credential", group: "auth", bucket: "safe", forbidSeverity: "medium", note: "'role' is not a credential-shaped name — a normal role check against a literal must NOT flag",
      code: `function isAdmin(user) {\n  return user.role === "admin";\n}\n` },
    { id: "auth-oauth-grant-type-password", group: "auth", bucket: "safe", forbidSeverity: "medium", note: "OAuth2's 'password' grant type — the compared identifier is 'grantType', not a credential-shaped name; this rule checks the NAME being compared, never the literal's content",
      code: `function handleGrant(grantType, credentials) {\n  if (grantType === "password") {\n    return authenticateWithCredentials(credentials);\n  }\n}\n` },
    { id: "auth-empty-password-check", group: "auth", bucket: "safe", forbidSeverity: "medium", note: "checking for an empty password is a normal validation pattern, not a backdoor — empty string literal is explicitly excluded",
      code: `function validate(password) {\n  if (password === "") return "Password is required";\n  return null;\n}\n` },
    { id: "auth-tokentype-bearer-scheme", group: "auth", bucket: "safe", forbidSeverity: "medium", note: "'tokenType' matches the credential-name pattern (contains 'token'), but 'Bearer' is an auth-scheme name, not a secret value — explicitly excluded",
      code: `function parseAuthHeader(tokenType, token) {\n  if (tokenType === "Bearer") {\n    return token;\n  }\n}\n` },
    { id: "auth-jwt-verify-used", group: "auth", bucket: "safe", forbidSeverity: "medium", note: "jwt.verify() is called elsewhere in the file — jwt.decode() must NOT be flagged when verification happens somewhere in the same file",
      code: `import jwt from 'jsonwebtoken';\nfunction authenticate(token) {\n  return jwt.verify(token, process.env.JWT_SECRET);\n}\nfunction peekClaims(token) {\n  return jwt.decode(token);\n}\n` },
    { id: "auth-unrelated-decode-not-jwt", group: "auth", bucket: "safe", forbidSeverity: "medium", note: "a .decode() call with no 'jsonwebtoken' reference anywhere in the file must NOT be flagged — this isn't a JWT-decoding rule for any decode() call",
      code: `function decodeMessage(encoder, value) {\n  return encoder.decode(value);\n}\n` },
    { id: "auth-test-assertion-not-flagged", group: "auth", bucket: "safe", forbidSeverity: "medium", note: "a test-assertion-library call (expect().toBe()) is a CallExpression, not a BinaryExpression === comparison — must NOT flag, by AST shape alone",
      code: `test('returns the stored password', () => {\n  const user = getUser();\n  expect(user.password).toBe("test123");\n});\n` },
