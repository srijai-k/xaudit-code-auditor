...
function computeMetrics(scored: ScoredCase[]): Metrics {
    ...
    const tp = nonParserCases.reduce((s, c) => s + c.tp.length, 0);
    const fp = nonParserCases.reduce((s, c) => s + c.fp.length, 0);
    const fn = nonParserCases.reduce((s, c) => s + c.fn.length, 0);
    const tn = nonParserCases.filter((c) => c.isTrueNegative).length;
...
function writeMetrics(name: "baseline" | "experimental", scored: ScoredCase[], metrics: Metrics) {
    ...
    console.log(`Wrote ${outPath} — TP=${metrics.tp} FP=${metrics.fp} FN=${metrics.fn} TN=${metrics.tn} precision=${fmt(metrics.precision)} recall=${fmt(metrics.recall)} F1=${fmt(metrics.f1)}`);
}
...
function writeComparison(groundTruth: GroundTruthCase[], baseScored: ScoredCase[], expScored: ScoredCase[], baseMetrics: Metrics, expMetrics: Metrics) {
    ...
    md += `| TP | ${baseMetrics.tp} | ${expMetrics.tp} | ${diff(baseMetrics.tp, expMetrics.tp)} |\n`;
    md += `| FP | ${baseMetrics.fp} | ${expMetrics.fp} | ${diff(baseMetrics.fp, expMetrics.fp)} |\n`;
    md += `| FN | ${baseMetrics.fn} | ${expMetrics.fn} | ${diff(baseMetrics.fn, expMetrics.fn)} |\n`;
    md += `| TN | ${baseMetrics.tn} | ${expMetrics.tn} | ${diff(baseMetrics.tn, expMetrics.tn)} |\n`;
    md += `| Precision | ${fmt(baseMetrics.precision)} | ${fmt(expMetrics.precision)} | ${diffF(baseMetrics.precision, expMetrics.precision)} |\n`;
    md += `| Recall | ${fmt(baseMetrics.recall)} | ${fmt(expMetrics.recall)} | ${diffF(baseMetrics.recall, expMetrics.recall)} |\n`;
    md += `| F1 | ${fmt(baseMetrics.f1)} | ${fmt(expMetrics.f1)} | ${diffF(baseMetrics.f1, expMetrics.f1)} |\n`;
    md += `| High/Critical findings | ${baseMetrics.highOrCriticalFindings} | ${expMetrics.highOrCriticalFindings} | ${diff(baseMetrics.highOrCriticalFindings, expMetrics.highOrCriticalFindings)} |\n`;
    md += `| Recognized-sanitizer (low) findings | ${baseMetrics.recognizedSanitizerFindings} | ${expMetrics.recognizedSanitizerFindings} | ${diff(baseMetrics.recognizedSanitizerFindings, expMetrics.recognizedSanitizerFindings)} |\n`;
    ...
    md += `| Ambiguous/unsupported cases correct (of ${baseMetrics.ambiguousCasesTotal}) | ${baseMetrics.ambiguousCasesCorrect} | ${expMetrics.ambiguousCasesCorrect} | ${diff(baseMetrics.ambiguousCasesCorrect, expMetrics.ambiguousCasesCorrect)} |\n\n`;
