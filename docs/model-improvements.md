...
## HTML mode now analyzes parseable inline `<script>` content (2026-09-04)

**The problem, in plain terms:** F-05 (`docs/self-audit-2026-09-03.md`) closed the silent-clean-report gap by adding `html-script-content-not-analyzed` — an honest disclosure that HTML mode never looks inside `<script>` blocks. That was the right fix at the time, but it left the actual gap open: a real bug inside an inline script — a hardcoded secret, an `eval()`, an unsanitized `innerHTML` — still required the user to notice the disclosure, manually copy the script content out, and re-paste it under JS/TS/React mode themselves. Confirmed live during manual testing of a realistic vulnerable HTML fixture: running it in HTML mode surfaced only hygiene findings and the disclosure, while the same script content, copied out and re-run under JS/TS/React mode, produced real XSS/secrets/dynamic-exec findings the first pass never touched.

**The fix:** `analyze.ts` now extracts every inline `<script>` block (`extractInlineScripts()` in `html.ts` — still pure text scanning, no HTML parser, keeping that module's own architectural boundary) and attempts `parseSource()` on each one independently. A block that parses gets run through the same five real rules (`SCRIPT_RULES`) JS/TS/React mode uses, with every finding's line number shifted by the block's actual starting line in the original document so it points at the real location, not line 1 of the extracted snippet. A block that can't parse standalone (relies on surrounding template syntax to be valid) falls back to the existing `html-script-content-not-analyzed` disclosure — now scoped to that one block, not the whole document, so a block that got real findings is never followed by a contradictory "this wasn't examined" note sitting next to them.

**Why this needed care, not just wiring the pieces together:** the old regression fixture for F-05 (`<template>{{ msg }}</template><script>export default { data() { return { msg: eval(location.hash) } } }</script>`) turned out to be a bad test of the disclosure path once this shipped — that script's content is actually valid, self-contained JavaScript (an `export default` object literal) and parses fine on its own, so it now correctly produces a real `exec-eval` finding instead of the disclosure. Kept as a live example of why the fallback path needed a *genuinely* unparseable fixture (one relying on template-engine syntax like `<% %>` that isn't valid JS by itself) to actually test the fallback, rather than one that happened to work by coincidence before this feature existed.

**How this was verified:** 4 new/updated tests in `tests/unit/html-rules.test.ts`, moved to exercise `analyze()` directly rather than `runHtmlChecks()` alone (the extraction/re-parse/remap orchestration lives in `analyze.ts`, not in the HTML module) — a parseable block with a real `eval()` surfaces `exec-eval` with the correctly remapped line number and no accompanying disclosure; a block relying on template syntax still gets the disclosure; multiple blocks in one document are handled independently (one gets a real secret finding, the other gets the disclosure); a `type="application/json"` block is never attempted as JavaScript at all. Re-verified live in the running app against a realistic HTML fixture: a script block with a hardcoded OpenAI-shaped key, an `innerHTML` sink, and an `eval()` call produced all three real findings in the same HTML-mode report as the hygiene checks, with the secret's line number matching its actual line in the pasted document.

**What this does *not* do:** still no cross-block or cross-file data-flow — each script block is analyzed in isolation, so a value split across two separate `<script>` tags isn't traced between them. A block that depends on ambient globals defined by surrounding template output (not just syntax) can still parse "successfully" while missing context the real deployment would have; this is the same category of limitation the top-level JS/TS/React mode already carries, not a new one introduced here.

**Full suite after this:** 213/213 unit tests passing (was 209).
