...
| 2 | `location.href = "javascript:" + userInput` | XSS-adjacent (javascript: URI) | Currently: **not flagged at all** — no rule inspects `location.href`/`window.location` assignment | **Gap — not covered by any rule** |
| 3 | `element.setAttribute('href', 'javascript:' + userInput)` | XSS-adjacent | Not flagged | **Gap** |
| 4 | `bcrypt.compareSync(plainPassword, storedPlaintext)` where `storedPlaintext` is never hashed | Auth | Not flagged — no auth rule exists at all | **Gap — entire vulnerability class unimplemented, and README already discloses this** |
| 5 | `if (password === storedPassword)` plain-text comparison | Auth | Not flagged | **Gap, disclosed** |
| 6 | `jwt.decode(token)` used where `jwt.verify(token, secret)` was needed (signature never checked) | Auth | Not flagged | **Gap, disclosed** |
...
Recommend adding items 2, 3, 11, and 13 to the fixture corpus with `bucket: "edge-cases"` and `forbidSeverity`/`expectRuleIds` reflecting the *current* (gap) behavior, the same way this project already tracks other known limitations — so these stay visible rather than silently rediscovered later.
